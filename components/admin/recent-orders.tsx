...
function getNextAction(status: OrderStatus): { action: Action; label: string } | null {
  if (status === "PENDING") return { action: "CONFIRM", label: "Confirm" };
  if (status === "CONFIRMED") return { action: "ASSIGN_PARTNER", label: "Assign Partner" };
  if (status === "PARTNER_ASSIGNED") return { action: "OUT_FOR_DELIVERY", label: "Out for Delivery" };
  if (status === "OUT_FOR_DELIVERY") return { action: "MARK_DELIVERED", label: "Mark Delivered" };
  return null;
}
...
const ACTION_MESSAGES: Record<Action, string> = {
  ...
  OUT_FOR_DELIVERY: "Marked out for delivery — email sent",
                    ...
                    <div className="flex gap-2 mt-2">
                      {next && (
                        <Button
                          ...
                          onClick={() => handleAction(order.id, next.action)}
                          disabled={!!actionLoading}
                        >
                          {actionLoading === order.id + next.action ? "..." : next.label}
                        </Button>
                      )}
                      {canCancel && (
                        <Button
