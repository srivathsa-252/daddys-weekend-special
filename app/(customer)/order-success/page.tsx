...
export default async function OrderSuccessPage({
  ...
  const order = orderId
    ? await prisma.order.findUnique({ where: { id: orderId }, select: { orderNumber: true, createdAt: true } })
    : null;
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          {displayNumber && (
            <div className="px-6 py-5 border-b border-gray-100 text-center">
              ...
              <p className="text-gray-300 text-xs font-mono mt-2 break-all">{orderId}</p>
            </div>
          )}
          ...
          <div className="px-6 py-5 space-y-4">
            ...
            <div className="space-y-3">
              {[
                { step: "1", text: "The restaurant reviews your order and sends a confirmation email." },
                { step: "2", text: "A delivery partner is assigned and you'll be notified by email." },
                { step: "3", text: "Your food is prepared and on its way to you!" },
              ].map(({ step, text }) => (
